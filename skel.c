...
uint32_t get_interface_ipaddr(int interface)
{
	struct ifreq ifr;
	sprintf(ifr.ifr_name, "r-%u", interface);
	ioctl(interfaces[interface], SIOCGIFADDR, &ifr);
...
//Functie ce verifica daca un IP este destinat routerului
int check_for_router(uint32_t dest_ip) {
	for (int i = 0; i < ROUTER_NUM_INTERFACES; i++) {
		if (get_interface_ipaddr(i) == dest_ip) {
			return 1;
			break;
		}
	}
